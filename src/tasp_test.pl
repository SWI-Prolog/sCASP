...
%:- include('pr/hanoi_pr.pl').                       query(X) :- ?? hanoi(3,X).
...
%:- include('tasp_examples/clp_fd_example_pr.pl').           query([X,Y]) :- ?? [p(X), go(X,Y)].

%% clpq examples
% :- include('tasp_examples/clpq_ex01_pr.pl').           query([X,Y]) :- ?? [p(X), go(X,Y)].
% :- include('tasp_examples/cross_train_pr.pl').           query([M,N,X]) :- ?? [findrange(M,N,X)].
%:- include('tasp_examples/ball_states_pr.pl').           query([T]) :- ?? [hold(T)].
%:- include('tasp_examples/ball_hamilton_pr.pl').           query([T]) :- ?? [reachable(0)].

%% continuous time systems
:- include('tasp_examples/ball_asp_pr.pl').           query([S,C,R]) :- ?? [path(s(s0,0,0),s(S,11,C),R)].
